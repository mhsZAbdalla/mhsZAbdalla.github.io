/* P5 summative, Honor, made by Zubair Abdalla
on June 3rd, 2019
*/
function preload() {
  ...
  mango = loadImage('textures/mango.png')
  strawberry = loadImage('textures/strawberry.png')
...
class Fruit {
  constructor(x,y,xvel,yvel) {
    ...
    this.lifespan = 255
    this.falling = false
    // Gives the fruit a random texture
    this.texture = random([apple,melon,pineapple,mango,strawberry])
  ...
  death() {
    ...
    // The player must 'swipe' their cursor over a fruit to slice it
    var mouseVel = abs(mouseX - pmouseX || mouseY - pmouseY)
    var d = dist(mouseX,mouseY,this.x,this.y)
    if (this.lifespan === 255 && d < 40 && mouseVel >= 5 && mouseInCanvas === true) {
      this.lifespan = 0
      score ++
      ...
      this.gravity = 0
      fruitsSliced ++
      // Plays the 'sword' sound effect
      sword.play()
    }
    // Gives the player a 'fruits missed' counter, when this reaches 3, the game ends.
    // Once the fruit goes below its original spawn point, it is counted as a missed fruit.
...
class Bomb {
  ...
  death() {
    ...
    var mouseVel = abs(mouseX - pmouseX || mouseY - pmouseY)
    var d = dist(mouseX,mouseY,this.x,this.y)
    if (d < 25 && mouseVel >= 5 && mouseInCanvas === true) {
      clear()
      level = 2
    }
  }
...
function setup() {
  ...
  frameRate(60)
  noCursor()
  // Trails can be found in the trail.js file
  trail1 = new Trail()
  trail2 = new Trail2()
  trail3 = new Trail3()
  trail4 = new Trail4()
...
function gameScreen() {
  ...
  // Game over if the player misses 3 fruits.
  if (fruitsMissed === 3) {
    level = 2
  }
} // end function
...
function gameShop() {
  ...
  shopItem(600,450,'Plasma Trail','10 Coins',5)
  shopDisplay(140,0,255,600,380,25)
